...
/**
 * @title MakeMerkle
 * @author Original work by kootsZhin (https://github.com/dmfxyz/murky)
 * @notice Updated to handle address, index, and amount in leaf nodes
 */
...
contract MakeMerkle is Script, ScriptHelper {
    ...
    string private elements =
        vm.readFile(string.concat(vm.projectRoot(), inputPath));
    ...
    function run() public {
        ...
        for (uint256 i = 0; i < count; ++i) {
            ...
            // keccak256(bytes.concat(keccak256(abi.encode(claimant, index, amount))))
            leafs[i] = keccak256(
